...
tree_bag::tree_bag(const tree_bag &src) {
	tree = reinterpret_cast<node*>(copy_node(src.tree));
}
...
void *tree_bag::copy_node(node *current) {
	if (current == NULL) {
		...
		new_node->l = reinterpret_cast<node*>(copy_node(current->l));
		new_node->r = reinterpret_cast<node*>(copy_node(current->r));
		return reinterpret_cast<void*>(new_node);
	}
}
